...
```{r}
cwd <- getwd()
methods <- c("CAMERA", "LeapR", "PaintOmics", "mBONITA")
PAlist <- vector(mode = "list", length = length(methods))
names(PAlist) <- c("CAMERA", "LeapR", "PaintOmics", "mBONITA")
PAlist[["CAMERA"]] <- read.csv(paste0(cwd, "/reactomeGSA-camera/camera_allResults.csv"), row.names = 1)
temp <- PAlist[["CAMERA"]]$FDR < 0.05 #PValue < 0.01
PAlist[["CAMERA"]] <- PAlist[["CAMERA"]][temp,]
PAlist[["CAMERA"]]$Pathway <- trimws(gsub("KEGG|_|\\d+$", " ", PAlist[["CAMERA"]]$GeneSet))
...
makeVisList <- function(tempPAlist, contrast) {
  ...
  visList[["CAMERA"]] <-
    unique(str_to_title(as.vector(tempPAlist[["CAMERA"]]$Pathway[tempPAlist[["CAMERA"]]$Method == "Proteomics"]))) #CHANGE
  visList[["LeapR"]] <-
    unique(str_to_title(as.vector(tempPAlist[["LeapR"]]$Pathway[tempPAlist[["LeapR"]]$Method == "Phosphoproteomics"]))) #NO COMBINED RESULTS
  visList[["PaintOmics"]] <-
...
upset(fromList(visList), order.by = c("freq", "degree"), mb.ratio = c(0.55, 0.45), empty.intersections = "off", point.size = 2, line.size = 1)
```
