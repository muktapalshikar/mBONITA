...
p1 = ggplot(data = as.data.frame(temp4)) + geom_histogram(aes(value, fill = variable), color = 'black') + theme_pubr(legend=c(0.7,0.75), margin=FALSE) + scale_fill_brewer('Contrast', palette = 'Blues') + ylab("Count of genes in the network") + xlab('Node modulation score')+ theme(text = element_text(size=8))
