...
p1 = upset(fromList(visList), order.by = c("freq", "degree"), mb.ratio = c(0.55, 0.45), empty.intersections = NULL, point.size = 5, line.size = 1 , mainbar.y.label = "Intersections", sets.x.label = "Pathways", text.scale = c(2,2, 2, 2, 2, 2), number.angles = 0, main.bar.color = 'black', sets.bar.color = 'black', shade.color = 'white', matrix.color = 'black') 

png(filename = paste0('upset','_mBonita_','allContrasts.png'), width = 8, height = 3, pointsize = 14, res = 600, units = 'in')
print(p1)
dev.off()
...
png(filename = "heatmap_mBonita_HIF1pathways.png", width = 12, height = 10, pointsize = 10, res = 600, units = 'in')
p1 
dev.off()
print(p1)
```
...
```{r}
temp4 = melt(as.data.frame(temp), value.vars = 'nodeModulation', id.vars = c('Contrast', 'index'))
#for (c in unique(temp4$Contrast)){
#  temp5 = temp4[temp4$Contrast == c,]
#  temp5$value = unlist(lapply(temp5$value, function(x){return(x/max(temp5$value))}))
#  temp4[temp4$Contrast == c,] = temp5
#}
# bin values > 2x the 3rd quartile
test = 2*summary(temp4$value)[5]
temp4$value = unlist(lapply(temp4$value, function(x){if(x > test){return(test)}else{return(x)}}))
p1 = ggplot(data = as.data.frame(temp4)) + geom_histogram(aes(value, fill = Contrast), color = 'black')+ theme_pubr(legend=c(0.7,0.75), margin=FALSE) + scale_fill_brewer('Contrast', palette = 'Blues') + ylab("Count of genes in the network") + xlab('Node modulation score')+ theme(text = element_text(size=8)) #+ scale_x_continuous(breaks=as.vector(summary(temp4$value))[1:4]) + xlim(0, as.vector(summary(temp4$value))[4])
