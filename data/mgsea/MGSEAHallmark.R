#Query gene sets
c1 <- read.csv("h.all.v6.1.symbols.gmt",sep ="\t",stringsAsFactors = FALSE, header = FALSE)

#Sorted gene list for PROT, PHOSPH and MRNA
fall <- read.csv("Top1000Genes.csv",stringsAsFactors = FALSE)

MGSEA <- function(fall, c1){
...
for(ii in 1:nrow(c1)){
  ...
  #Number of observed cancer genes when considering feautre 1 (F1 = PROT) only
  f1aonly <- numeric(length(steps))
  for(i in steps){
    f1aonly[i] <- sum(f1a[1:i] %in% target)
  }

  #Number of observed cancer genes when considering feature 2 (F2 = PHOSPHhylation) only
  ...
  dat1 <- data.frame(
    ...
    Feature = c(rep("Combined",length(steps)),rep("PROT", length(steps)),rep("PHOSPH", length(steps)),rep("PHOSPH|PROT", length(steps)),rep("PROT|PHOSPH", length(steps)),rep("Null", length(steps)))
  )
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PROT", "PHOSPH","PHOSPH|PROT","PROT|PHOSPH","Null"))
  PROTPHOSPH1 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count") +
    ggtitle("PROT vs PHOSPH",subtitle = "A")+ scale_color_manual(values=c("red","yellow","magenta","green","blue","black"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))

  # PROT&PHOSPH VS MRNA
  dat1 <- data.frame(
    Rank = (c(steps,steps,steps,steps)),
    CanGenes = (c( overlap,f1f2aonly,f3aonly,f1f2)),
    Feature = c(rep("Combined",length(steps)),rep("PROT&PHOSPH", length(steps)),rep("MRNA", length(steps)),rep("MRNA|PROT&PHOSPH", length(steps)))
  )
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PROT&PHOSPH", "MRNA","MRNA|PROT&PHOSPH"))
  PROTPHOSPH2 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count") +
    ggtitle("PROT&PHOSPH vs MRNA",subtitle = "B")+ scale_color_manual(values=c("red","yellow","magenta","green"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))
  ...
  dat1 <- data.frame(
    ...
    Feature = c(rep("Combined",length(steps)),rep("PROT", length(steps)),rep("MRNA", length(steps)),rep("MRNA|PROT", length(steps)),rep("PROT|MRNA", length(steps)),rep("Null", length(steps)))
  )
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PROT", "MRNA","MRNA|PROT","PROT|MRNA","Null"))
  PROTmrna1 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count") +
    ggtitle("PROT vs MRNA",subtitle = "E")+ scale_color_manual(values=c("red","yellow","magenta","green","blue","black"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))

  #PROT&MRNA VS PHOSPH
  dat1 <- data.frame(
    Rank = (c(steps,steps,steps,steps)),
    CanGenes = (c( overlap,f1f3aonly,f2aonly,f1f3)),
    Feature = c(rep("Combined",length(steps)),rep("PROT&MRNA", length(steps)),rep("PHOSPH", length(steps)),rep("PHOSPH|PROT&MRNA", length(steps))))
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PROT&MRNA", "PHOSPH","PHOSPH|PROT&MRNA"))
  PROTmrna2 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count") +
    ggtitle("PROT&MRNA vs PHOSPH",subtitle = "F")+ scale_color_manual(values=c("red","yellow","magenta","green"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))

  #PHOSPH VS MRNA
  dat1 <- data.frame(
    Rank = (c(steps,steps,steps,steps,steps,steps)),
    CanGenes = (c( f2f3aonly,f2aonly,f3aonly,f3gf2,f2gf3,expest)),
    Feature = c(rep("Combined",length(steps)),rep("PHOSPH", length(steps)),rep("MRNA", length(steps)),rep("MRNA|PHOSPH", length(steps)),rep("PHOSPH|MRNA", length(steps)),rep("Null", length(steps)))
  )
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PHOSPH", "MRNA","MRNA|PHOSPH","PHOSPH|MRNA","Null"))
  PHOSPHmrna1 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count") +
    ggtitle("PHOSPH vs MRNA",subtitle = "C")+ scale_color_manual(values=c("red","yellow","magenta","green","blue","black"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))

  #PHOSPH&MRNA VS PROT
  dat1 <- data.frame(
    Rank = (c(steps,steps,steps,steps)),
    CanGenes = (c( overlap,f2f3aonly,f1aonly,f2f3)),
    Feature = c(rep("Combined",length(steps)),rep("PHOSPH&MRNA", length(steps)),rep("PROT", length(steps)),rep("PROT|PHOSPH&MRNA", length(steps)))
  )
  dat1$Feature<- factor(dat1$Feature, levels = c("Combined","PHOSPH&MRNA", "PROT","PROT|PHOSPH&MRNA"))
  PHOSPHmrna2 <- ggplot(data=dat1, aes(x=Rank, y=CanGenes, group=Feature, color=Feature)) +
    geom_line() + theme(legend.text=element_text(size=8))+ylab("Feature Count")+
    ggtitle("PHOSPH&MRNA vs PROT",subtitle = "D")+ scale_color_manual(values=c("red","yellow","magenta","green"))+geom_line(size=1)+theme(plot.title = element_text(hjust = 0.5))

  name1 <- gsub(" ", "", c1gname)

  #Corresponding plots
  title1 <- paste("PROTPHOSPH", name1, "1.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PROTPHOSPH1)
  dev.off()

  title1 <- paste("PROTPHOSPH", name1, "2.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PROTPHOSPH2)
  dev.off()


  title1 <- paste("PROTMRNA", name1, "1.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PROTmrna1)
  dev.off()

  title1 <- paste("PROTMRNA", name1, "2.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PROTmrna2)
  dev.off()

  title1 <- paste("PHOSPHMRNA", name1, "1.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PHOSPHmrna1)
  dev.off()


  title1 <- paste("PHOSPHMRNA", name1, "2.pdf",sep = "")
  pdf(title1, width = 15,height = 15)
  print(PHOSPHmrna2)
  dev.off()
}
