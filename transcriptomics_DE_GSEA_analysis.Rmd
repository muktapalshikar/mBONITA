...
ha = HeatmapAnnotation(type = type, border = TRUE, show_annotation_name = 0, which = "column", 
    ...
    col = list(type = c(`19% O2` = "white", `19% O2 + CyA` = "grey", `1% O2` = "black", 
        `1% O2 + CyA` = "grey89")))
pdf('allDEgenes_scaled_heatmap.pdf', width = 9, height = 20)
DEheatmap(scaledHeatmap, name = "Z-score\n", colGroups = ha, cluster_columns = TRUE, 
    cluster_rows = TRUE, show_column_names = FALSE, column_km = 3, border = TRUE, row_km=5)
dev.off()
png('allDEgenes_scaled_heatmap.png', width = 9, height = 20, units="in", res=300)
DEheatmap(scaledHeatmap, name = "Z-score\n", colGroups = ha, cluster_columns = FALSE, 
    cluster_rows = TRUE, show_column_names = FALSE, column_km = 3, border = TRUE, row_km=5)
dev.off()
DEheatmap(scaledHeatmap, name = "Z-score\n", colGroups = ha, cluster_columns = FALSE, 
...
```{r}
require(ggthemes)
enrichPlot = ggplot(data=allEnrich) + geom_point(aes(x = log10pvalue, y = reorder(ID, -log10pvalue))) + facet_wrap(vars(Contrast), as.table=TRUE, nrow=3, scales = "free_y") + theme_pander(nomargin = TRUE, fc = "black", gM=TRUE, gm=TRUE, gc = "black", boxes=TRUE, gl = "dotdash") + xlab("-log10 (p-value)") + ylab("")+ theme(strip.background = element_rect(fill = "cornsilk"), strip.placement = "outside")
enrichPlot
ggsave(filename = "all_enrichr_results.pdf", plot = enrichPlot, width = 7, height=7)
ggsave(filename = "all_enrichr_results.png", plot = enrichPlot, width = 7, height=7)
```
