...
#for i in range(0, 3): # iterate over pathways
    print(i)
    pathway = pathList[i]
    pathImportances = pd.DataFrame.from_dict(pathway[1], orient = 'index')
...
for j in range(0, len(contrasts)): # iterate over contrasts
    ...
    #for i in range(0, 3):
        ...
        if len(nodeMod_complete) == 0:
            nodeMod_complete = nodeModulation_pathway
        else:
        ...
        for k in range(1000):
            ...
            nm_temp = nm_temp.sum(axis=1) * pd.DataFrame(score.T.sample(n = len(pathImportances.index))).reset_index(drop=True)
            nm_temp = nm_temp[[0]]
            #print(nm_temp)
            nm_temp = (pd.DataFrame(pathImportances.reset_index(drop=True)) * nm_temp).values.sum()
            #print(nm_temp)
        ...
        if stdev == 0:
            ...
            zscore = (nodeModulation_pathway.nodeModulation.sum() - meaner) / stdev
