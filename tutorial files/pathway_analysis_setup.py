    ...
    if len(lines.split("\n")[0].split(":")) > 1:
        ...
        if len(list(nx.connected_components(graph.to_undirected()))) > 0:
            ...
            if len(genes.intersection(list(graph.nodes()))) > 1:
                with open(str(coder) + ".gpickle", 'wb') as f:
                ...
                pickle.dump(pathwaySampleList, open(str(coder) + "_sss.pickle", "wb"))
    else:
        print("not found:")
        print(requester)
        print(lines)
    ...
    for x in pathwayList:
        ...
        if (
            ...
            if (
                ...
                nx.write_graphml(graph, str(coder)  + ".graphml")  # write graph out
                with open(str(coder) + ".gpickle", 'wb') as f:
                    pickle.dump(graph, f, pickle.HIGHEST_PROTOCOL)
                print(
                    (
...
if __name__ == "__main__":
    ...
    if customNetwork != "False":
        print("Using the custom network " + str(customNetwork))
